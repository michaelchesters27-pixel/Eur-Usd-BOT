...
{
   return "E5C" + IntegerToString(g_campaign_number) + "|" + side + IntegerToString(level);
}
...
void ProtectEarlierPositions(const string side)
{
   ...
   double offset = MathMax(0.0, InpBreakevenOffsetPips) * PipSize();
...
bool PlaceCampaignOrders()
{
   ...
   double first_distance = MathMax(InpFirstOrderDistancePips * pip, broker_minimum);
   double spacing = MathMax(InpOrderSpacingPips * pip, _Point);
   double take_profit_distance = MathMax(InpIndividualTakeProfitPips * pip, broker_minimum);
...
int OnInit()
{
   ...
   if(StringLen(_Symbol) < 6 || StringSubstr(_Symbol, 0, 6) != "EURUSD")
   {
      Alert("This EA is for EURUSD only.");
      return INIT_FAILED;
   }
   if((ENUM_ACCOUNT_MARGIN_MODE)AccountInfoInteger(ACCOUNT_MARGIN_MODE) != ACCOUNT_MARGIN_MODE_RETAIL_HEDGING)
   {
      Alert("The EURUSD 5x5 bot requires an MT5 hedging account.");
      return INIT_FAILED;
   }
   if(InpLotSize <= 0 || InpFirstOrderDistancePips <= 0 || InpOrderSpacingPips <= 0 ||
      InpIndividualTakeProfitPips <= 0 || InpCampaignTargetMoney <= 0)
   {
   ...
   g_gv_prefix = "E5X5_" + IntegerToString((int)InpMagicNumber) + "_" + _Symbol + "_";
